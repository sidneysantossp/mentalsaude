...
const connectionString = process.env.DATABASE_MIGRATION_URL || process.env.POSTGRES_DATABASE_URL;
if (!connectionString) {
  console.error("Nenhuma URL de banco PostgreSQL encontrada.");
  process.exit(1);
}
...
async function seed() {
  try {
    const existing = await sql`SELECT id FROM assessments WHERE slug = 'pcl-5' LIMIT 1`;
    const pcl5Data = {
      slug: "pcl-5",
      title: "Lista de Verificação de TEPT para o DSM-5 (PCL-5)",
      category: "Trauma e Estresse",
      shortDescription: "Instrumento de 20 perguntas para autoavaliação e rastreio de sintomas de estresse pós-traumático.",
      description: "O PCL-5 é um instrumento de autorrelato validado para mensurar a gravidade dos 20 sintomas de TEPT descritos no DSM-5-TR.",
      durationMinutes: 5,
      difficulty: "Moderada",
      status: "publicado",
      scoringGuide: JSON.stringify({
        ...
        cutOff: 33,
        bands: [
          { max: 32, label: "Baixo Indicador de Sintomas de TEPT", summary: "Suas respostas indicam pouca ou nenhuma frequência de sintomas pós-traumáticos significativos no último mês." },
          { max: 80, label: "Atenção Prioritária (Sintomas Elevados de TEPT)", summary: "Suas respostas apontam para uma pontuação elevada de sintomas relacionados ao trauma. Recomenda-se buscar avaliação especializada." }
        ]
      }),
    ...
    if (existing.length === 0) {
      await sql`
        INSERT INTO assessments (slug, title, category, "shortDescription", description, "durationMinutes", difficulty, status, "scoringGuide", questions)
        VALUES (${pcl5Data.slug}, ${pcl5Data.title}, ${pcl5Data.category}, ${pcl5Data.shortDescription}, ${pcl5Data.description}, ${pcl5Data.durationMinutes}, ${pcl5Data.difficulty}, ${pcl5Data.status}, ${pcl5Data.scoringGuide}, ${pcl5Data.questions})
      `;
      console.log("PCL-5 inserido com sucesso no banco PostgreSQL.");
    } else {
      await sql`
        UPDATE assessments SET title = ${pcl5Data.title}, category = ${pcl5Data.category}, "shortDescription" = ${pcl5Data.shortDescription}, description = ${pcl5Data.description}, "durationMinutes" = ${pcl5Data.durationMinutes}, difficulty = ${pcl5Data.difficulty}, status = ${pcl5Data.status}, "scoringGuide" = ${pcl5Data.scoringGuide}, questions = ${pcl5Data.questions}
        WHERE slug = 'pcl-5'
      `;
      console.log("PCL-5 atualizado com sucesso no banco PostgreSQL.");
    }
  } catch (err) {
    console.error("Erro ao semear PCL-5:", err);
  } finally {
    await sql.end();
  }
}
