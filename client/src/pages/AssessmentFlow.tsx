...
import { trackAnxietyTestCompletion } from "@/lib/analytics";
...
type Result = { score: number; percentage: number; band: string; summary: string; displayValue?: string; metricLabel?: string; recommendations: Recommendation[]; savedToPanel?: boolean };
...
function AssessmentContent({ assessmentId }: { assessmentId: number }) {
  ...
  const completionEventTrackedRef = useRef(false);
...
export function ResultScreen({ result, title, onFinish }: { result: Result; title: string; onFinish: () => void }) {
  ...
  return <div className="min-h-screen bg-[#f7f6ef] px-5 py-12 text-[#153a36] sm:px-8">
    <main className="mx-auto max-w-[820px] space-y-6">
      {result.savedToPanel && <div role="status" aria-live="polite" className="flex items-start gap-3 rounded-2xl border border-[#b8ded5] bg-[#effaf6] px-5 py-4 text-sm text-[#0a615a]"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0" aria-hidden="true" /><div><strong className="font-semibold">Resultado salvo com sucesso.</strong><p className="mt-1 text-[#3f6d64]">A devolutiva já está disponível no seu painel em “Meus testes”.</p></div></div>}
