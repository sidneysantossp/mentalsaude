...
import { ArrowLeft, ArrowRight, Clock3, Gauge, Info, ListChecks } from "lucide-react";
...
export default function TestCatalog() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f6ef] text-[#153a36]">
      <header className="mx-auto flex h-20 max-w-[1240px] items-center justify-between px-5 sm:px-8"><div className="flex items-center gap-6"><Link href="/"><Brand /></Link><Link href="/conteudos" className="text-sm font-semibold text-[#3b5d56] transition-colors hover:text-[#0b7167]">Conteúdos</Link></div><Button variant="ghost" className="text-[#0a615a]" onClick={() => setLocation("/")}><ArrowLeft className="mr-2 h-4 w-4" />Voltar ao início</Button></header>
      ...
      <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{isLoading && Array.from({ length: 3 }).map((_, index) => <div key={index} className="h-72 animate-pulse rounded-3xl bg-[#e5efeb]" />)}{data?.map((assessment, index) => <article key={assessment.id} className="group relative overflow-hidden rounded-3xl border border-[#dce9e4] bg-[#fffefa] p-6 shadow-[0_20px_45px_-37px_rgba(12,65,58,.8)]"><div className="absolute -right-8 -top-9 h-32 w-32 rounded-full bg-[#d8f0ea] transition-transform duration-300 group-hover:scale-110" /><div className="relative"><Badge className="border-0 bg-[#e6f4f0] text-[#0a7066] hover:bg-[#e6f4f0]">{assessment.category}</Badge><h2 className="mt-8 font-display text-2xl font-semibold tracking-[-.035em] text-[#143e39]">{assessment.title}</h2><p className="mt-3 min-h-12 text-sm leading-6 text-[#67827b]">{assessment.shortDescription}</p><div className="mt-7 flex items-center gap-4 border-y border-[#e5eeeb] py-3 text-xs font-semibold text-[#618179]"><span className="flex items-center gap-1.5"><Clock3 className="h-3.5 w-3.5 text-[#0b7167]" />{assessment.durationMinutes} min</span><span className="flex items-center gap-1.5"><Gauge className="h-3.5 w-3.5 text-[#0b7167]" />{difficultyLabel[assessment.difficulty]}</span></div><Button className="mt-6 w-full rounded-xl bg-[#0b675f] text-white hover:bg-[#084e49]" onClick={() => { if (user) { setLocation(`/avaliacao/${assessment.id}`); return; } sessionStorage.setItem("mental-saude:selected-assessment", String(assessment.id)); beginLogin(); }}>Fazer autoavaliação <ArrowRight className="ml-2 h-4 w-4" /></Button></div></article>)}
