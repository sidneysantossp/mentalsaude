...
import { Brain, Heart, Flame, Activity, Plus, FileText, TrendingUp, Users, Calendar, BarChart3, Clock, Target, LineChart, PieChart } from 'lucide-react'
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h1 className="text-2xl font-bold text-gray-900">
                  {session ? `Bem-vindo(a), ${session.user?.name || 'Usuário'}!` : 'Seu Dashboard'}
                </h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              ...
              <div className="bg-white shadow rounded-lg p-6">
                ...
                <div className="h-64">
                  <SimplePieChart 
                    data={Object.entries(
                      historicalData.reduce((acc: any, item) => {
                        const key = item.testCategory
                        acc[key] = (acc[key] || 0) + 1
                        return acc
                      }, {})
                    ).map(([name, value]) => ({
                      name: name === 'DEPRESSION' ? 'Depressão' : 
                            name === 'ANXIETY' ? 'Ansiedade' : 
                            name === 'ADHD' ? 'TDAH' : 
                            name === 'STRESS' ? 'Estresse' : name,
                      value: value as number,
                      fill: name === 'DEPRESSION' ? '#3b82f6' : 
                            name === 'ANXIETY' ? '#10b981' : 
                            name === 'ADHD' ? '#f59e0b' : 
                            name === 'STRESS' ? '#ef4444' : '#6b7280'
                    }))} 
                  />
                </div>
              </div>
