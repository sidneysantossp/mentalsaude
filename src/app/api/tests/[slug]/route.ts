...
export async function GET(
  ...
  try {
    ...
    const { data: test, error } = await db
      .from('tests')
      .select(
        'id, slug, title, description, short_description, category, instructions, time_limit, card_image, is_premium, is_active, questions(id, text, type, options, order)'
      )
      .eq('slug', slug)
      .maybeSingle()
    ...
    if (test) {
      return NextResponse.json(formatTest(test))
    }
